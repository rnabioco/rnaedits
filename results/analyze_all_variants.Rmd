...
output:
  html_document:
    ...
    code_folding: hide
params:
  allele_counts: "../../data/vcf/variant_allele_counts_by_strand/A_G_alleles/counts/"
  variant_info: "../../data/vcf/variant_allele_counts_by_strand/A_G_alleles/filtered_select_variants.bed.gz"
  ref: "A"
  alt: "G"
  prefix: "../2017-06-18/variant_allele_counts_by_strand/"
  dirname: "A_G_alleles"
---
...
system(paste0("mkdir -p ", paste0(prefix, "/", outname, "/")))

bind_cols <- c('#chrom', "pos", "ref", "base")
all_variants_export <- Reduce(function(x, y) inner_join(x, y, by = bind_cols), dat)
write_gztsv(all_variants_export, paste0(prefix, "/", outname, "/all_sites_raw_counts.txt"))
```
...
write_gztsv(all_prop, paste0(prefix, "/", outname, "/", 
                            ref, "_", alt, "_all_sites_proportions.txt"))

write_gztsv(all_long, paste0(prefix, "/", outname, "/", 
                            ref, "_", alt, "_all_sites_raw_counts_wide.txt"))

write_gztsv(all_dat_export, paste0(prefix, "/", outname, "/", 
                            ref, "_", alt, "_all_sites_raw_counts.txt"))
...
write_gztsv(filtered_prop, paste0(prefix, "/", outname, "/", 
                            ref, "_", alt, "_filtered_sites_proportions.txt"))

write_gztsv(filtered_long, paste0(prefix, "/", outname, "/", 
                            ref, "_", alt, "_filtered_sites_raw_counts_wide.txt"))

write_gztsv(filtered_dat_export, paste0(prefix, "/", outname, "/", 
                            ref, "_", alt, "_filtered_sites_raw_counts.txt"))
...
```{r}
saveRDS(edger_objs, "edgeR_de_editing.rds")
edger_objs <- readRDS("edgeR_de_editing.rds")
...
write_tsv(output_warm_cold, paste0(prefix, "/", outname, "/", 
                            ref, "_", alt,       "warm_v_cold_post_hoc_edger.txt"))
```
...
map2(cpms, names(cpms), ~as.data.frame(.x) %>% 
       tibble::rownames_to_column("site") %>% 
       write_tsv(paste0(.y, "_normalized_cpms.txt")))
```
...
pdf(paste0(prefix, "/", outname, "/", ref, "_", alt, "sig_editing_sites_clustered_rnames.pdf"), useDingbats = F)
tryCatch({get_sig_heatmap(all_sites, all_prop_dat_genes, 
                      title = paste0("Significant ", ref, "->", alt, " SNVs \n n = "), 
                      pdata_all,
                      row_names_gp = gpar(fontsize = 8),
...
pdf(paste0(prefix, "/", outname, "/", ref, "_", alt, "sig_editing_sites_not_clustered_rnames.pdf"), useDingbats = F)
tryCatch({get_sig_heatmap(all_sites, all_prop_dat_genes, 
                      title = paste0("Significant ", ref, "->", alt, " SNVs \n n = "), 
                      pdata_all,
                      row_names_gp = gpar(fontsize = 8),
...
pdf(paste0(prefix, "/", outname, "/", ref, "_", alt, "sig_editing_sites_clustered.pdf"), useDingbats = F)
tryCatch({get_sig_heatmap(all_sites, all_prop_dat_genes, 
                      title = paste0("Significant ", ref, "->", alt, " SNVs \n n = "), 
                      pdata_all,
                      row_names_gp = gpar(fontsize = 8),
...
pdf(paste0(prefix, "/", outname, "/", ref, "_", alt, "sig_editing_sites_not_clustered.pdf"), useDingbats = F)
tryCatch({get_sig_heatmap(all_sites, all_prop_dat_genes, 
                      title = paste0("Significant ", ref, "->", alt, " SNVs \n n = "), 
                      pdata_all,
                      row_names_gp = gpar(fontsize = 8),
...
plot_all_sig_genes <- function(prop_dat, toptag_dat, stat_region = "Medulla", region_to_plot = "all",
  ...
  gene_labels <- plot_dat %>% dplyr::select(GeneName) %>% unlist()
  names(gene_labels) <- plot_dat %>% dplyr::select(site) %>%  unlist()
...
write_gztsv(long_proportion_dat, paste0(params$prefix, "/", params$dirname, "/", params$ref, "_", params$alt, "_filtered_sites_proportions_tidy.txt"))

write_gztsv(exon_sum_tidy, paste0(params$prefix, "/", params$dirname, "/", params$ref, "_", params$alt, "_mean_editing.txt"))

write_gztsv(tt_exons_annotated, paste0(params$prefix, "/", params$dirname,  "/", params$ref, "_", params$alt, "_filtered_sites_annotation.txt"))
...
pdf(paste0(prefix, "/", outname,  "/", ref, "_", alt, "impactful_sig_editing_sites_clustered_rnames.pdf"), useDingbats = F)
tryCatch({get_sig_heatmap(all_sites, all_prop_dat_genes, 
                      title = paste0("Significant ", ref, "->", alt, " SNVs \n n = "), 
                      pdata_all,
                      row_names_gp = gpar(fontsize = 8),
...
pdf(paste0(prefix, "/", outname, "/", ref, "_", alt, "impactful_sig_editing_sites_not_clustered_rnames.pdf"), useDingbats = F)
tryCatch({get_sig_heatmap(all_sites, all_prop_dat_genes, 
                      title = paste0("Significant ", ref, "->", alt, " SNVs \n n = "), 
                      pdata_all,
                      row_names_gp = gpar(fontsize = 8),
...
pdf(paste0(prefix, "/", outname, "/", ref, "_", alt, "impactful_sig_editing_sites_clustered.pdf"), useDingbats = F)
tryCatch({get_sig_heatmap(all_sites, all_prop_dat_genes, 
                      title = paste0("Significant ", ref, "->", alt, " SNVs \n n = "), 
                      pdata_all,
                      row_names_gp = gpar(fontsize = 8),
...
pdf(paste0(prefix, "/", outname, "/", ref, "_", alt, "impactful_sig_editing_sites_not_clustered.pdf"), useDingbats = F)
tryCatch({get_sig_heatmap(all_sites, all_prop_dat_genes, 
                      title = paste0("Significant ", ref, "->", alt, " SNVs \n n = "), 
                      pdata_all,
                      row_names_gp = gpar(fontsize = 8),
...
write_gztsv(long_proportion_dat, paste0(params$prefix, "/", params$dirname, "/", params$ref, "_", params$alt, "_impactful_sites_proportions_tidy.txt"))

write_gztsv(exon_sum_tidy, paste0(params$prefix, "/", params$dirname, "/", params$ref, "_", params$alt, "_impactful_mean_editing.txt"))

write_gztsv(impact_exons_annotated, paste0(params$prefix, "/", params$dirname, "/", params$ref, "_", params$alt, "_impactful_sites_annotation.txt"))
