...
```{r basic_stats}
fc_stats <- read_tsv(file.path(data_dir, "featurecounts/dexseq/count_summary.tsv.summary"))
...
# get top 10 exons DE in all regions
sig_exons %>%  
  group_by(exonID) %>% 
  mutate(total = n()) %>% 
...
reordered_cols <- state_cols[c(5, 3, 2, 4, 1, 6)]
DEXSeq::plotDEXSeq(dexresults$forebrain, 
                   "G18716", 
                   fitExpToVar = "State",
                   legend = T,
                   color= reordered_cols)

DEXSeq::plotDEXSeq(dexresults$hypothalamus, 
                   "ENSSTOG00000024849", 
                   fitExpToVar = "State",
                   legend = T,
                   color= reordered_cols)
...
walk(top10, function(x){
  pdf(paste0(x, "_dexseq_plot.pdf"))
  DEXSeq::plotDEXSeq(dexresults$hypothalamus, 
                   x, 
                   fitExpToVar = "State",
                   legend = T,
