...
all_annot_site_names <- bind_rows(all_annot_site_names, .id = "allele") %>% 
  group_by(edit_site) %>% 
  summarize(alleles = dplyr::first(allele))
...
file.copy("edits/all_filtered_variant_sites_enrichment.pdf", 
          sfigs_dir[2], overwrite = T)
...
b
 ...
 file.copy("edits/all_sig_sites_annotated_proportions.pdf",
...
file.copy("hyperedits/predicted_effects.pdf",
...
file.copy("edits/ZCCHC8.pdf",
          sfigs_dir[15], overwrite = TRUE)
...
plot_sites(prop_dat = long_prop_dat, 
           ...
           title = "edits/all_validation_sites.pdf", 
           base_width = 16, 
           base_height = 8)
...
plot_sites(prop_dat = long_prop_dat, 
           ...
           title = "edits/newly_annotated_sites.pdf", 
           base_width = 16, 
           base_height = 8)
...
```{r}
# tidy pdata
pdata <- readr::read_tsv(file.path(docs_dir,
...
file.copy("edits/GRIA2_htrc2_boxplot.pdf",
          sfigs_dir[3])
