...
file.copy("edits/wss_plot.pdf", 
          file.path(sfigs_dir[4],
                    "wss_plot_edits.pdf"),
                    overwrite = F)
```
...
file.copy("edits/heatmap_subset_of_impactful_sites.pdf", sfigs_dir[12], overwrite = T)
```
...
```{r, conserved_sites, fig.width = 6, fig.height = 8, eval = F}
# editing frequencies
...
walk2(all_sig_sites_matrix, names(all_sig_sites_matrix), 
      function(x, y) {
        ...
        plot_nonag(x, pdata, y)
        dev.off()
        file.copy(paste0("edits/", y, "_heatmap.pdf"),
                  sfigs_dir[5])
      }
)
