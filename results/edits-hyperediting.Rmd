...
file.copy("hyperedits/all_alleles_all_regions.pdf",
          sfigs_dir[9], overwrite = T) 
```
...
file.copy("hyperedits/all_sites.pdf",
          sfigs_dir[9], overwrite = T)
all_regions %>% 
  ungroup() %>% 
  mutate(total_sites = sum(total),
...
file.copy("hyperedits/ag_sites_per_state_mean_line.pdf",
...
file.copy("hyperedits/all_alleles_all_regions_clusters.pdf",
          sfigs_dir[9], overwrite = T)
```
...
dev.off() 
file.copy("hyperedits/allregions_shared_hyperedits.pdf",
          sfigs_dir[9], overwrite = T)
plot(fit, 
     fill = state_cols,
     fill_opacity = 0.55,
...
variant_info <- file.path(data_dir,
                          "hyperedits",
                          "hyperedited_sites_annotated.bed.gz")
...
file.copy("hyperedits/rotated_heatmap_k2.pdf",
          sfigs_dir[4], overwrite = T)
```
...
file.copy("hyperedits/wss_plot.pdf", 
          sfigs_dir[4], overwrite = T)
```
...
file.copy("hyperedits/rotated_heatmap_k3.pdf",
          figs_dir[2], overwrite = T)
```
...
file.copy("hyperedits/editing_index_across_states_by_cluster_kmeans.pdf",
          figs_dir[2], overwrite = T)
...
pdf("hyperedits/gatk_vs_hyperedits_euler.pdf",
    width = 4, 
    height = 4)
file.copy("hyperedits/gatk_vs_hyperedits_euler.pdf",
...
file.copy("hyperedits/gatk_vs_hyperedits_euler_nonsig.pdf",
          figs_dir[3], overwrite = T)
...
file.copy("hyperedits/editing_index_torpor_enriched_v_constitutive.pdf",
          figs_dir[3], overwrite = T)
```
